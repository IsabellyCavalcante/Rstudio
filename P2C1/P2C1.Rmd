...
A visualização que escolhi para refazer foi baseada em uma resposta de uma atividade minha passada referente a distribuição das porcentagens de passagens gastas com pessoas que não fossem o próprio deputado.
...
```{r}
despesas_por_trimestre <- function(mes1, mes2, mes3, ano, dados){
  dados = dados %>%
    filter(numAno == ano) %>%
  ...
  dados_trimestre_final = merge(dados1, passagens_total, by.x = "ideCadastro", by.y = "ideCadastro.y") %>%
...
despesas_15_t1 = despesas_por_trimestre(1, 2, 3, 2015, gastos_aviao)

despesas_15_t2 = despesas_por_trimestre(4, 5, 6, 2015, gastos_aviao)

despesas_15_t3 = despesas_por_trimestre(7, 8, 9, 2015, gastos_aviao)

despesas_15_t4 = despesas_por_trimestre(10, 11, 12, 2015, gastos_aviao)
